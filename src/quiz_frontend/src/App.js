...
class App {
  ...
  #handleSubmit = async (e) => {
    ...
    this.greeting = await quiz_backend.greet(name);
    this.#render();
  };
...
function onOptionSelect(e) {
    ...
    if (optionSelected.textContent === correctAnswer) {
        optionSelected.style.backgroundColor = "green"
        incrementScore()
    } else {
        optionSelected.style.backgroundColor = "red"
    }
    
    // Wait for 1 second before loading the next question to show feedback
...
function loadOptions(ques) {
    ...
    quesOptions.forEach(function(item) {
        let li = document.createElement("li")
        li.textContent = item
        li.style.backgroundColor = "" // Reset background color for new options
        options.appendChild(li)
    })
}
